...
class VerticalSliceTest(unittest.TestCase):
    def compile_typescript(self):
        subprocess.run([resolve_node_command("npm"), "run", "test:ts"], cwd=ROOT, check=True, capture_output=True, text=True)

    def run_vertical_slice(self):
        subprocess.run([resolve_node_command("npm"), "run", "vertical-slice"], cwd=ROOT, check=True, capture_output=True, text=True)
